...
def highlight_edges(edges):
    ...
    highlited_edges = cv2.dilate(edges, np.ones((3, 3), np.uint8), iterations=1, borderType=cv2.BORDER_CONSTANT)
...
def order_corners(corners):
    ...
    bottom_right = corners[np.argmin(diff_corners)]#smallest difference of coordinates y - x
    bottom_left = corners[np.argmin(sum_corners)]#smallest sum of coordinates x + y
